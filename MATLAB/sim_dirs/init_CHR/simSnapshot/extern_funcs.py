...
class OCVlookup(daeScalarExternalFunction):
    def __init__(self, Name, Model, units, c):
        arguments = {}
        arguments["c"] = c
        arr = np.genfromtxt('/home/amitchell/Dropbox/Graduate/Research/Code/Matlab/MPET/data/C100avg.csv', delimiter=',')
        self.interp = sintrp.interp1d(arr[:,0], arr[:,1], bounds_error=False,
                                      fill_value="extrapolate")
        daeScalarExternalFunction.__init__(self, Name, Model, units, arguments)

    def Calculate(self, values):
        c = values["c"]
        cval = float(self.interp(c))
        return adouble(cval)
