    ...
    def sld_dynamics_1D2var(self, c1, c2, muO, act_lyte, ISfuncs, noises):
        ...
        if ndD["type"] in ["diffn2", "CHR2"]:
            ...
            if ndD["side"] == 1:
                ...
                #eq.Residual = self.dcsidedt() -  3 * ((1-w1)*self.Rxn1() + (1-w2)*self.Rxn2())
                #(3/ndD["psd_mean"])   
...
def OCV_surf(muR_ref,c_surf):
    OCV = (1.0949 + 1.2227*np.exp(-350.37*c_surf) +
           0.28168*np.tanh(-(c_surf -0.054591)/0.067063) -
           0.072685*np.tanh((c_surf -0.63682 )/0.66244)  -
           0.72244*np.tanh((c_surf + 0.053614)/0.095513) -
           0.016717*np.tanh((c_surf -0.2724)/0.033756))
