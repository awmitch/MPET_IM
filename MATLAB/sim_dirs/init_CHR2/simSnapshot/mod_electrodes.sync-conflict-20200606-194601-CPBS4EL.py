...
class Mod2var(dae.daeModel):
    def __init__(self, Name, Parent=None, Description="", ndD=None,
        ...
        self.mueff = dae.daeVariable("mueff", dae.no_t, self, "Effective surface chemical potential")
        # Ports
    ...
    def DeclareEquations(self):
        ...
        r_vec, volfrac_vec = geo.get_unit_solid_discr(ndD['shape'], N, ndD['Ar'])
    ...
    def sld_dynamics_1D2var(self, c1, c2, muO, act_lyte, ISfuncs, noises):
        ...
        Mmat = get_Mmat(ndD['shape'], N, Ar)
        dr, edges = geo.get_dr_edges(ndD['shape'], N, Ar)

        # Get solid particle chemical potential, overpotential, reaction rate
        if ndD["type"] in ["diffn2", "CHR2"]:
            (mu1R, mu2R), (act1R, act2R) = calc_muR(
                                                    (c1, c2), (self.c1bar(), self.c2bar()), T, ndD, ndD_s, ISfuncs)#
            c1_surf = c1[-1]
            c2_surf = c2[-1]
            if ndD["OCV_flag"]:
                mu1R_surf = OCV_surf(ndD["muR_ref"][-1],c1_surf)
                mu2R_surf = OCV_surf(ndD["muR_ref"][-1],c2_surf)
            else:
                mu1R_surf = mu1R[-1]
                mu2R_surf = mu2R[-1]
            act1R_surf = act1R[-1]
        ...
        if ndD["type"] in ["diffn2", "CHR2"]:
            ...
            eq = self.CreateEquation("mueff")
            eq.Residual = self.mueff() - (mu1R_surf + mu2R_surf)/2
            if ndD["side"] == 1:
                ...
                w1 = np.tanh(ndD["gamma_side"] * (1-c1_surf))
                w2 = np.tanh(ndD["gamma_side"] * (1-c2_surf))
                eq.Residual = self.dcsidedt() -  3/4 * ((1-w1)*self.Rxn1() + (1-w2)*self.Rxn2())
                #(3/ndD["psd_mean"])   
                Flux1_bc = -0.5 * self.Rxn1() * w1
                Flux2_bc = -0.5 * self.Rxn2() * w2
...
#                    c1, c2, ndD["D"], Flux1_bc, Flux2_bc, dr, T)
            ...
            elif ndD["shape"] == "ellipsoid":
                if Ar > 1.0:
                    e=(1.-(1./Ar)**2)**0.5
                    area_vec = 2.*np.pi*(((1./Ar)*(edges))**2) + (2.*np.pi*(edges)**2/Ar/e)*np.arcsin(e)
                elif Ar < 1.0:
                    e=(1.-Ar**2)**0.5
                    area_vec = 2.*np.pi*(edges**2)/(Ar**2) + (np.pi*(edges)**2/e)*np.log((1+e)/(1-e))
                elif Ar == 1.0:
                    area_vec = (4*np.pi)*(edges)**2
...
class Mod1var(dae.daeModel):
    ...
    def sld_dynamics_1D1var(self, c, muO, act_lyte, ISfuncs, noise):
        ...
        if ndD["type"] in ["ACR"]:
            ...
            c_surf = c[-1]
            if ndD["OCV_flag"]:
                muR_surf = OCV_surf(ndD["muR_ref"][-1],c_surf)
            else:
                muR_surf = muR[-1]
            actR_surf = actR[-1]
        eta = calc_eta(muR_surf, muO)
...
def get_Mmat(shape, N, Ar):
    r_vec, volfrac_vec = geo.get_unit_solid_discr(shape, N, Ar)
    if shape == "C3":
        Mmat = sprs.eye(N, N, format="csr")
    elif shape in ["sphere", "cylinder","ellipsoid"]:
        Rs = 1.
...
def calc_muR(c, cbar, T, ndD, Ar, ISfuncs=None):
    muRfunc = props_am.muRfuncs(T, ndD, Ar).muRfunc
    muR_ref = ndD["muR_ref"]
    muR, actR = muRfunc(c, cbar, muR_ref, Ar, ISfuncs)
    return muR, actR
...
def OCV_surf(muR_ref,c_surf):
    ...
    #OCV = (38.28+1.092*np.exp(-13.48*c_surf)+0.006323*np.tanh(-(c_surf-0.2713)/0.01853)-10.57*np.tanh((c_surf-1.237)/0.1439)-20.1*np.tanh((c_surf+0.008967)/0.005118)-0.01105*np.tanh((c_surf-0.4917)/0.03674)-0.02754*np.tanh((c_surf-0.1782)/-0.02144)-0.03982*np.tanh((c_surf-0.02377)/-0.006442)+28.73*np.tanh((c_surf+0.3573)/-0.02075))
