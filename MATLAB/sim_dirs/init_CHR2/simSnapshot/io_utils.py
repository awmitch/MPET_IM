...
def get_dicts_from_configs(P_s, P_e):
    ...
    if P_e.get("a") == 1:
        if P_e["a"].get('Particles', 'shape') =="ellipsoid" and P_e["c"].get('Particles', 'shape') =="sphere":
            try:
                ndD_s["Ar"] = {"a": P_s.getfloat('Particles', 'Ar_a'), "c":1}
            except:
                ndD_s["Ar"] = {"a": 1., "c": 1.}
        elif P_e["a"].get('Particles', 'shape') =="sphere" and P_e["c"].get('Particles', 'shape') =="ellipsoid":
            try:
                ndD_s["Ar"] = {"a":1., "c": P_s.getfloat('Particles', 'Ar_c')}
            except:
                ndD_s["Ar"] = {"a": 1., "c": 1.}
        elif P_e["a"].get('Particles', 'shape') =="ellipsoid" and P_e["c"].get('Particles', 'shape') =="ellipsoid":
            try:
                ndD_s["Ar"] = {"a": P_s.getfloat('Particles', 'Ar_a'), "c": P_s.getfloat('Particles', 'Ar_c')}
            except:
                ndD_s["Ar"] = {"a": 1., "c": 1.}
        else:
            ndD_s["Ar"] = {"a": 1., "c": 1.}
    ...
    for trode in ndD_s["trodes"]:
        aR= ndD_s["Ar"][trode]
        try:
            MRD=P_s.getfloat('Particles', 'MRD')
        except:
            MRD = 100000.0
        S= (2.+MRD-3.*(MRD/(MRD-1.))**0.5*np.arccosh(MRD**0.5))/(MRD-1.)
        if aR > 1.0:
            Lx = (aR/2.)*((aR/(aR**2-1.))-(np.arccosh(aR)/(aR**2-1.)**1.5))
        elif aR < 1.0:
            Lx = (aR/2.)*((aR/(aR**2-1.))+(np.arccos(aR)/(1.-aR**2)**1.5))
        else:
            Lx = 1./3.
        Brugg = np.around(-(1.+2.*S+3.*Lx*(-1.-2.*S+2.*Lx))/(6.*Lx-6*Lx**2),4)
        ndD_s["BruggExp"][trode]= Brugg
    ...
    for trode in ndD_s["trodes"]:
        ...
        ndD_e[trode]["psd_mean"] = dD_s["psd_mean"][trode]
        ndD_e[trode]["L_ref"] = L_ref
        ...
        Ar = ndD_s["Ar"][trode]
        muRfunc = props_am.muRfuncs(ndD_s["T"], ndD_e[trode], Ar).muRfunc
        ...
        if Type in ndD_s["2varTypes"]:
            ndD_e[trode]["muR_ref"] = -muRfunc(cs0, cs0bar, Ar, 0.)[0][0]
        elif Type in ndD_s["1varTypes"]:
            ndD_e[trode]["muR_ref"] = -muRfunc(cs0, cs0bar, Ar, 0.)[0]
    ...
    for trode in ndD_s["trodes"]:
        ...
        if solidShape == "sphere":
            ...
            psd_vol[trode] = (np.pi * psd_len[trode]**2
                              * dD_e[trode]['thickness'])
        elif solidShape == "ellipsoid":
            if Ar > 1.0:
                e=(1.-(1./Ar)**2)**0.5
                psd_area[trode] = 2.*np.pi*(1./Ar*(psd_len[trode])**2) + (2.*np.pi*(psd_len[trode])**2/Ar/e)*np.arcsin(e)
                psd_vol[trode] = (4./3)*np.pi*(psd_len[trode])**3/Ar**2
            elif Ar < 1.0:
                e=(1.-Ar**2)**0.5
                psd_area[trode] = 2.*np.pi*(psd_len[trode])**2/Ar**2 + (np.pi*(psd_len[trode])**2/e)*np.log((1+e)/(1-e))
                psd_vol[trode] = (4./3)*np.pi*(psd_len[trode])**3/Ar**2
            elif Ar == 1.0:
                psd_area[trode] = (4*np.pi)*(psd_len[trode])**2
                psd_vol[trode] = (4./3)*np.pi*(psd_len[trode])**3
            ...
            curlyK =psd_area[trode]*psd_len[trode]/psd_vol[trode]
...
def test_electrode_input(dD, ndD, dD_s, ndD_s):
    ...
    if solidShape not in ["C3", "sphere", "cylinder","ellipsoid"]:
