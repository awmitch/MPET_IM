...
def get_unit_solid_discr(Shape, N, Ar):
    if N == 1:  # homog particle, hopefully
        ...
        dr = Rs/(N - 1)
        r_vec = np.linspace(0, Rs, N)
        vol_vec = 4*np.pi*(Ar**2)*(r_vec**2 * dr + (1./12)*dr**3)
        vol_vec[0] = 4*np.pi*(Ar**2)*(1./24)*dr**3
        vol_vec[-1] = (4./3)*np.pi*(Ar**2)*(Rs**3 - (Rs - dr/2.)**3)
        Vp = 4./3.*np.pi*(Ar**2)*Rs**3
        volfrac_vec = vol_vec/Vp
    else:
...
def get_dr_edges(shape, N, Ar):
    r_vec = get_unit_solid_discr(shape, N, Ar)[0]
    dr = edges = None
...
#        curv[N-1] = (2*c[-2] - 2*c[-1] + 2*dr*beta_s)/dr**2
        ...
        raise NotImplementedError("calc_curv_c only for sphere, cylinder and ellipsoid")
    return curv
